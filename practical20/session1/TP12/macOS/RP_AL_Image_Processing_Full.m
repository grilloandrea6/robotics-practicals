...
% 3) Estimate the excitation frequency and amplitude of each degree of
% freedom (i.e., rotation - motor 1 and translation - motor 2).
...
prompt = {'The real frame rate (FPS)', 'Excitation frequency f1 (Hz)', 'Excitation frequency f2 (Hz)' };
dlgtitle = File;
dims = [1 100]; definput = {'50','1','1'}; answer = inputdlg(prompt,dlgtitle,dims,definput);
Fs = str2double(answer{1});     % Real Frame rate
Fex1 = str2double(answer{2});   % Hz - Excitation frequency - f1
Fex2 = str2double(answer{3});   % Hz - Excitation frequency - f2
...
OriantationTc = double(A0(ixTOc,iyTOc));        % Oriantation Top marker region
%% 
OriantationBc = double(A0(ixBOc,iyBOc));        % Oriantation Bottom marker region
SmallP = double(A0(ixSP,iySP));                 % Small particle tracking region

%% Estimate the particle radius 
GeneralP = double(A0(ixGP,iyGP));
figure(1); clf 
imshow(GeneralP,A_lim, 'InitialMagnification', magnification*10);
drawnow
...
while hasFrame(v)
    ...
    O_angle_rad(i_frame) = atan2( (-Pos_pixTG(2)+Pos_pixBG(2)),(-Pos_pixTG(1)+Pos_pixBG(1)) );      % levitator angle
...
F_est = fminbnd(@(ff) Linear_sine_fit(ff,tau,detrend(U_pos_mm_ss)/max(abs(detrend(U_pos_mm_ss)))), 0.5*Fex2*TSCALE, 1.5*Fex2*TSCALE);
...
plot(timeV,[UV_pos_mm_shift(:,1) Amat_uv*coefs_u-DC_u, U_pos_mm_ss-(Amat_uv*coefs_u)],'--o')
    ...
    title(sprintf('Fitted frequency: %.3f Hz, Amplitude: %.3f mm, DC: %.3f mm',Fex_ident_u,Amp_u,DC_u))
subplot(2,1,2)
...
% Estimate the frequency using NL-LS
F_est = fminbnd(@(ff) Linear_sine_fit(ff,tau,detrend(O_angle_rad_ss)/max(abs(detrend(O_angle_rad_ss)))), 0.7*Fex1*TSCALE, 1.3*Fex1*TSCALE);
...
yline(DC_th/pi*180,'r')
    ...
    title(sprintf('Fitted frequency: %.3f Hz, DC offset: %.3f (\\circ), Amplitude: %.3f (\\circ)',Fex_ident_th,DC_th/pi*180,Amp_th/pi*180))
%% 3) Get the origin position (u,v) and Track the particle in (r,z) with respect to the origin
...
plot(timeV,z,'--o'); 
    ...
    ylabel('z (mm)')
subplot(2,2,3)
plot(timeV,[Pos_Mat_mm(:,1) C_v(:,1) ],'--o'); 
    xlabel('t (s)')
    ylabel('X (mm)')
    legend('Particle','center')
subplot(2,2,4)
plot(timeV,[Pos_Mat_mm(:,2) C_v(:,2) ],'--o'); 
    xlabel('t (s)')
    ylabel('Y (mm)')
    legend('Particle','center')
...
if SAVE==1
    ...
    save([path  Filenew],'timeV','Fex_ident_u','Fex_ident_th','Pos_Mat_mm','C_v','r','z','O_angle_rad','Amp_v','Amp_u','Amp_th','DC_u','DC_v','DC_th','Particle_Rad_mm');
end
